...
class Net(object):
    ...
    def backward(self, X, y, lamda):
        ...
        answer,pred= self.activations, self.a
        ...
        for (w,b,a) in reversed(list(zip(self.weights,self.biases,answer))):
            # Compute gradient for the relu function
            out = np.dot(a,w) + b.T
            # If the layer has only one neuron dont compute gradient for relu (output layer)
            if out.shape[1] != 1:
                delY = delY * (out > 0)
